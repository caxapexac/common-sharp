...
namespace Caxapexac.Common.Sharp.MathExtra
{
    ...
    public static class MathFast
    {
        ...
        static MathFast()
        {
            ...
            {
                _sinCache[i] = (float)Math.Sin((i + 0.5f) / SinCosCacheSize * Pi2);
                _cosCache[i] = (float)Math.Cos((i + 0.5f) / SinCosCacheSize * Pi2);
            }

            var factor = SinCosCacheSize / 360f;
            for (i = 0; i < 360; i += 90)
            {
                _sinCache[(int)(i * factor) & SinCosIndexMask] = (float)Math.Sin(i * Pi / 180f);
                _cosCache[(int)(i * factor) & SinCosIndexMask] = (float)Math.Cos(i * Pi / 180f);
            }
            ...
            {
                _atan2CachePpy[i] = (float)Math.Atan(i * invAtan2Size);
        ...
        {
            return (float)Math.Pow(data, power);
        }
