...
using Caxapexac.Common.Sharp.Collections;
using Caxapexac.Common.Sharp.Data.XmlInternal;
using Caxapexac.Common.Sharp.Extensions;


namespace Caxapexac.Common.Sharp.Data
{
    /// <summary>
    ...
    namespace XmlInternal
    {
        ...
        internal static class XmlUtils
        {
            ...
            public static int SkipValue(string str, ref int i, char endChar, char endChar2, bool stopOnSpace)
            {
                var start = i;
                while (
                    (!stopOnSpace || !(str[i] == ' ' || str[i] == '\t' || str[i] == '\n' || str[i] == '\r')) && str[i] != endChar && str[i] != endChar2)
                {
                    i++;
                }
                return i - start;
