...
namespace Caxapexac.Common.Sharp.Editor.Window.Project
{
    /// <summary>
    ...
    internal sealed class GoogleDocsDownloader : EditorWindow
    {
        ...
        [MenuItem("Tools/Common/Google Docs Downloader")]
        ...
        private static string ConvertToDictJson(string data)
        {
            ...
            {
                if (!(
                    wrapperValue[i] == string.Empty || wrapperValue[i] == "[]" || wrapperValue[i] == "{}" || string.Compare(wrapperValue[i], "IGNORE", true) == 0 || wrapperValue[i] == "\"\""))
                {
            ...
            while (it.MoveNext())
            {
                ...
                for (var i = 0; i < headerValue.Length; i++)
                {
                    ...
                    itemValue = wrapChars.Length > 0 ? $"{wrapChars[0]}{it.Current.Value[i]}{wrapChars[1]}" : it.Current.Value[i];
        ...
        private static string ConvertToArrayJson(string data)
        {
            ...
            while (it.MoveNext())
            {
                ...
                for (var i = 0; i < headerValue.Length; i++)
                {
                    ...
                    itemValue = wrapChars.Length > 0 ? $"{wrapChars[0]}{it.Current[i]}{wrapChars[1]}" : it.Current[i];
                    sb.AppendFormat("{0}\"{1}\":{2}", needFieldsComma ? "," : string.Empty, headerValue[i], itemValue);
                    needFieldsComma = true;
                }
                sb.Append("}");
                ...
                using (var www = new WebClient())
                {
                    ...
                    foreach (var item in items)
                    {
                        if (!string.IsNullOrEmpty(item.Url) && !string.IsNullOrEmpty(item.Asset))
                        {
                            ...
                            {
                                throw new Exception($"\"{item.Url}\": {urlEx.Message}");
                            }
                            var path = $"{Application.dataPath}/{item.Asset}";
